...
# 毎リリースで中身が変わるのに URL が不変。CDN エッジが旧版を Cache-Control の max-age 分
# (実測 14400 秒 = 4 時間) 保持するため、パージしないと新規ダウンロード・自動更新が旧バージョンを
# 掴む。アップロード直後に該当 URL をパージして伝播を確定する。バージョン付き nupkg は URL が
# 一意 (= 旧キャッシュが存在しない) ためパージ不要。manifest 等の固定名のみ対象にする。
Write-Host '== Cloudflare キャッシュパージ ==' -ForegroundColor Cyan
$cfHeaders = @{ Authorization = "Bearer $($env:CLOUDFLARE_API_TOKEN)" }
$zoneName = ([uri]$BaseUrl).Host -replace '^[^.]+\.', ''   # exlsxs.nephilim.jp → nephilim.jp
